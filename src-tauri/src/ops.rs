//! Vault-touching domain operations: create/update/delete for every entity,
//! the referential-integrity cleanup on goal/notebook deletion, and
//! attachments. Factored out of `commands.rs` so this logic is plain
//! `fn(vault: &Path, ...)` — callable (and testable) without a Tauri
//! `AppHandle` (ADR-0006). Each `#[tauri::command]` is a one-line adapter:
//! `ops::x(&vault::require_vault(&app)?, ...)`.
...
/// Read the whole vault into one typed graph, waiting first for any
/// iCloud-evicted files in the loaded subfolders to download (`commands::load_all`
/// runs this off the main thread so that wait never freezes the window).
pub fn load_all(vault: &Path) -> AppResult<StoreSnapshot> {
...
pub fn create_task(vault: &Path, input: CreateTaskInput) -> AppResult<Task> {
    let task = Task {
        id: new_id(),
        title: input.title,
...
pub fn update_task(vault: &Path, mut task: Task) -> AppResult<Task> {
...
pub fn delete_task(vault: &Path, id: &str) -> AppResult<()> {
...
pub fn create_note(vault: &Path, input: CreateNoteInput) -> AppResult<Note> {
    let now = now_utc();
    let note = Note {
        id: new_id(),
        title: input.title,
...
pub fn update_note(vault: &Path, mut note: Note, body: String) -> AppResult<Note> {
...
pub fn delete_note(vault: &Path, id: &str) -> AppResult<()> {
    store_io::move_to_trash(vault, EntityKind::Note, id)?;
    store_io::move_attachment_path_to_trash(vault, &Path::new("attachments").join(id))
}

/// Read `note`'s current body, apply `edit` to its (already-loaded) metadata,
/// rewrite the `.md` with the edited metadata and the original body, and
/// return the updated metadata. Whether `updated` gets bumped is `edit`'s
/// call — the notebook/goal cleanup rewrites below intentionally leave it
/// alone; `move_note` bumps it.
fn rewrite_note_meta(vault: &Path, mut note: Note, edit: impl FnOnce(&mut Note)) -> AppResult<Note> {
...
pub fn move_note(vault: &Path, id: &str, notebook_id: Option<String>) -> AppResult<Note> {
    let note = store_io::read_note_meta(vault, id)?;
    rewrite_note_meta(vault, note, |n| {
...
pub fn create_notebook(vault: &Path, input: CreateNotebookInput) -> AppResult<Notebook> {
    let now = now_utc();
...
pub fn update_notebook(vault: &Path, mut notebook: Notebook) -> AppResult<Notebook> {
...
pub fn delete_notebook(vault: &Path, id: &str) -> AppResult<NotebookDeletionResult> {
...
fn clear_notebook_notes(vault: &Path, notebook_id: &str) -> AppResult<Vec<String>> {
    ...
    for note in store_io::load_notes(vault) {
        if note.notebook_id.as_deref() == Some(notebook_id) {
            let updated = rewrite_note_meta(vault, note, |n| n.notebook_id = None)?;
...
pub fn create_goal(vault: &Path, input: CreateGoalInput) -> AppResult<Goal> {
    let now = now_utc();
    let goal = Goal {
        id: new_id(),
        title: input.title,
...
pub fn update_goal(vault: &Path, mut goal: Goal) -> AppResult<Goal> {
...
pub fn delete_goal(vault: &Path, id: &str) -> AppResult<GoalDeletionResult> {
...
fn clear_linked_notes(vault: &Path, goal_id: &str) -> AppResult<Vec<String>> {
    ...
    for note in store_io::load_notes(vault) {
        if note.goal_id.as_deref() == Some(goal_id) {
            let updated = rewrite_note_meta(vault, note, |n| n.goal_id = None)?;
            cleared.push(updated.id);
        }
    }
    Ok(cleared)
...
pub fn attach_files(vault: &Path, entity_id: &str, paths: &[String]) -> AppResult<Vec<Attachment>> {
    let now = now_utc();
    paths
        .iter()
...
/// filename.
pub fn attach_bytes(vault: &Path, entity_id: &str, name: &str, bytes: &[u8]) -> AppResult<Attachment> {
...
pub fn remove_attachment(vault: &Path, path: &str) -> AppResult<()> {
...
mod tests {
    ...
    fn temp_vault() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("notes-goals-ops-test-{pid}-{n}"));
    ...
    fn delete_goal_clears_links_and_trashes_goal() {
        ...
        let result = delete_goal(&vault, "g1").unwrap();
    ...
    fn delete_notebook_unfiles_notes_and_preserves_them() {
        ...
        let result = delete_notebook(&vault, "nb1").unwrap();
    ...
    fn delete_task_without_attachments_folder_still_succeeds() {
        ...
        delete_task(&vault, "t-no-attachments").unwrap();
    ...
    #[test]
    fn move_note_rewrites_notebook_id_bumps_updated_and_preserves_body() {
        let vault = temp_vault();
        let note = Note {
            id: "n1".into(),
            title: "Note".into(),
        ...
        let moved = move_note(&vault, "n1", Some("nb1".into())).unwrap();

        assert_eq!(moved.notebook_id.as_deref(), Some("nb1"));
        assert_ne!(moved.updated, "2026-06-01T00:00:00Z", "updated should bump");
        assert_eq!(store_io::read_note_body(&vault, "n1").unwrap(), "body text");
    }
